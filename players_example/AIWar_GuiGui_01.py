...
hdlr = logging.handlers.RotatingFileHandler('./AIWar_GuiGui_01.log', mode='a', maxBytes=1000000, backupCount=1, encoding=None, delay=0)
...
logger.critical('Lancement de AIWar_GuiGui_01 !')
...
def play_miningship_test(ship):
    logger.error( "coucou, je suis play_miningship" )

    ship.setMemoryFloat(2, -5000)

    logger.error( ship.getMemoryInt(2) )
    logger.error( ship.getMemoryUInt(2) )
    logger.error( ship.getMemoryFloat(2) )

    n = ship.neighbours()
    for i in n:
        if isinstance(i, aiwar.Mineral):
            logger.error( "j'ai vu un mineral, j'y vais !" )
            ship.rotateTo(i)
            ship.move()
            logger.error( "extracted: %s", ship.extract(i) )
            logger.error( "mineralStorage: %s", ship.mineralStorage() )

        elif isinstance(i, aiwar.Missile):
            logger.error( "J'ai vu un missile !!!" )
            logger.error( "Il est a %s de moi !!!", ship.distanceTo(i) )

        elif isinstance(i, aiwar.MiningShip):
            logger.error( "MiningShip repere" )
            logger.error( "Ami ? %s", ship.isFriend(i) )

        elif isinstance(i, aiwar.Base):
            logger.error( "Base repere" )
            logger.error( "Amie ? %S", ship.isFriend(i) )
...
def play_base_test(self):
    ...
    for i in n:
        if isinstance(i, aiwar.MiningShip) and self.isFriend(i) and self.distanceTo(i) <= aiwar.BASE_REFUEL_RADIUS():
            logger.error( "je fais le plein de mon ami" )
            self.refuel(aiwar.MININGSHIP_START_FUEL(), i)
            

    # create new MiningShip
    if self.life() > (2*aiwar.BASE_FIGHTER_PRICE()) and random.randint(1, 20) == 1:
    ...
    posEnnemySelf = me[0]
    if posEnnemySelf != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and random.randint(1, 5) == 1:
        logger.error( "Je fabrique un guerrier1" )
        self.createFighter()


    # S'il existe un ennemi2 localise, creer un guerrier
    posEnnemySelf = me[1]
    if posEnnemySelf != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and random.randint(1, 5) == 1:
        logger.error( "Je fabrique un guerrier2" )
        self.createFighter()
    ...
    for i in n:
        if isinstance(i, aiwar.MiningShip) and self.isFriend(i) and self.distanceTo(i) <= aiwar.COMMUNICATION_RADIUS():
            friend = getFriendMiningShipMemory(self, i)
            if me[1] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[2] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and in_circle( friend[2][0], friend[2][1], aiwar.MININGSHIP_DETECTION_RADIUS()/4, me[0][0], me[0][1]):
...
def play_miningship_1(self):
    ...
    for i in n:
        if isinstance(i, aiwar.Fighter) and self.isFriend(i) and self.distanceTo(i) <= aiwar.COMMUNICATION_RADIUS():
            ...
            # Qui est plus loin de la BASE (donc plus proche de l'ennemi)
            # Et dont l'ennemi est le meme avec une precision de MININGSHIP_DETECTION_RADIUS()/4
            # Me.2
            if me[2] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID):
                friend = getFriendFighterMemory(self, i)
                fx,fy = friendpos = i.pos()
                mx,my = mypos = self.pos()
                bx,by = me[3]
                if me[2] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[0] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and in_circle( friend[0][0], friend[0][1], aiwar.MININGSHIP_DETECTION_RADIUS()/4, me[2][0], me[2][1]) and square_distance(mx,my , bx,by)<square_distance(fx,fy , bx,by):
                #if friend[0] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[0] == me[2] and i.missiles() > 0:
                    # Effacer l'ennemi de ma memoire
                    self.setMemoryFloat(2, CONST_ENNEMY_VOID + CONST_ENNEMY_VOID / CONST_Y_DIVIDE)
                    # Mettre a jour mon "Me"
                    me = getMyMiningShipMemory(self)
                if me[2] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[1] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and in_circle( friend[1][0], friend[1][1], aiwar.MININGSHIP_DETECTION_RADIUS()/4, me[2][0], me[2][1]) and square_distance(mx,my , bx,by)<square_distance(fx,fy , bx,by):
                #if friend[1] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[1] == me[2] and i.missiles() > 0:
                    # Effacer l'ennemi de ma memoire
                    self.setMemoryFloat(2, CONST_ENNEMY_VOID + CONST_ENNEMY_VOID / CONST_Y_DIVIDE)
                    # Mettre a jour mon "Me"
                    me = getMyMiningShipMemory(self)
...
#    logger.info( "Copier memoire Fighter : fin" )
    ...
    for i in n:
        if isinstance(i, aiwar.Base) and self.isFriend(i) and self.distanceTo(i) <= aiwar.COMMUNICATION_RADIUS():
            ...
            if me[2] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID):
                friend = getFriendFighterMemory(self, i)
                if me[2] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[0] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and in_circle( friend[0][0], friend[0][1], aiwar.MININGSHIP_DETECTION_RADIUS()/4, me[2][0], me[2][1]):
                #if friend[0] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[0] == me[2]:
                    # Effacer l'ennemi de ma memoire
                    self.setMemoryFloat(2, CONST_ENNEMY_VOID + CONST_ENNEMY_VOID / CONST_Y_DIVIDE)
                    # Mettre a jour mon "Me"
                    me = getMyMiningShipMemory(self)
                if me[2] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[1] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and in_circle( friend[1][0], friend[1][1], aiwar.MININGSHIP_DETECTION_RADIUS()/4, me[2][0], me[2][1]):
                #if friend[1] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID) and friend[1] == me[2]:
                    # Effacer l'ennemi de ma memoire
                    self.setMemoryFloat(2, CONST_ENNEMY_VOID + CONST_ENNEMY_VOID / CONST_Y_DIVIDE)
                    # Mettre a jour mon "Me"
                    me = getMyMiningShipMemory(self)
...
#                    me = getMyMiningShipMemory(self)
    ...
    if self.mineralStorage() == aiwar.MININGSHIP_MAX_MINERAL_STORAGE() or self.fuel() < ( self.distanceTo(basePos) if baseConnue else 170 ) or ennemiEnVue:
        if baseConnue:
            ...
            if self.distanceTo(basePos) > (aiwar.MININGSHIP_MINING_RADIUS()/2):
                self.move()
...
def play_fighter_test(self):
    ...
    if me[0] != (CONST_ENNEMY_VOID, CONST_ENNEMY_VOID):
        ...
        if self.missiles() <= 0:
            ...
            self.move()
        # Se deplacer tant qu'on est loin de la cible
        elif self.distanceTo(me[0]) > (aiwar.MININGSHIP_MINING_RADIUS()/2):
            self.rotateTo(me[0])
            self.move()
...
def play_fighter_1(self):
    logger.error( "" )
    logger.error( "********FIGHTER******** : (%s, %s)", int(self.pos()[0]), int(self.pos()[1]) )
    logger.error( "Vie: %s\t-\tFuel: %s\t-\tMissiles", self.life(), self.fuel(), self.missiles() )
